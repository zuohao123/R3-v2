#!/usr/bin/env python3
"""Build OCR-VQA unified JSONL with pseudo_text and OCR."""
...
def _raw_ocr_text(raw: Dict[str, Any]) -> str:
    for key in ("ocr_text", "ocr", "ocr_tokens", "tokens"):
        if key in raw and raw[key] not in (None, ""):
            return _ensure_text(raw[key], "")
    return ""


def main() -> None:
    parser = argparse.ArgumentParser(description="Build OCR-VQA unified JSONL.")
    parser.add_argument("--raw_jsonl", required=True)
    parser.add_argument("--ocr_cache", required=False)
