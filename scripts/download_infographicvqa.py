"""Download and export InfoVQA data from HuggingFace datasets."""
...
def _get_split(ds: Dataset | DatasetDict, split_name: str) -> Dataset:
...
def export_split(ds: Dataset, split: str, out_dir: str, log_every: int, num_proc: int) -> None:
    """Export one split into images and JSONL."""
    images_dir = os.path.join(out_dir, "images")
    raw_path = os.path.join(out_dir, f"infovqa_raw_{split}.jsonl")
    total = len(ds)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download InfoVQA dataset.")
    ...
    train_ds = load_dataset("vidore/infovqa_train")
    test_ds = load_dataset("vidore/infovqa_test_subsampled")

    export_split(_get_split(train_ds, "train"), "train", args.out_dir, args.log_every, args.num_proc)
    export_split(_get_split(test_ds, "test"), "test", args.out_dir, args.log_every, args.num_proc)
